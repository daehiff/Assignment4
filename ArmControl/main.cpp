...
Pose pose_from_centroid(tuple<double, double, double> centroid, int z, Vec2 scaling, Vec2 camera_orig_world) {
    int x = (int)get<0>(centroid);
    int y = (int)get<1>(centroid);
    Vec2 worldp = transform_to_world_coords(Vec2(x,y), scaling, camera_orig_world);
    return Pose((int) worldp.x, (int) worldp.y, z, (int)get<2>(centroid), 0, 180);
}
...
int moveArm(SOCKET ClientSocket, Pose pose) {
  int degX = pose.x + 90; // the rotation in robot coords and camera coords is off by 90 degrees
  cout << "Moving robot to pose: [" << pose.x << ", \t" << pose.y << ", \t" << pose.z
    << ", \t" << degX << "deg, \t" << pose.degY << "deg, \t" << pose.degZ << "]" << endl;

  //part 1
  char* commandPtr = new char[100];
  sprintf(commandPtr, "MOVP %d %d %d %d %d %d", pose.x, pose.y, pose.z - 35, degX, pose.degY, pose.degZ);
...
int stackBlocks(SOCKET ClientSocket, Pose bottomPose, Pose middlePose, Pose topPose) {
    goHome(ClientSocket);
    controlGripper(ClientSocket, GRIPPER_OPEN);
    moveArm(ClientSocket, pose_top);
    Sleep(1000);
	  Pose pose_down = pose_from_centroid(centroids[pickup_id], -215, scaling, camera_orig_world);
    moveArm(ClientSocket, pose_down);
...
int __cdecl main(int argc, char **argv) {
    ...
    Vec2 * known_points_cam = new Vec2[64];
    Vec2 * known_points_world = new Vec2[64];
    read_known_points(&known_points_count, known_points_cam, known_points_world);

    cout << "Estimating origin..." << endl;
    ...
    vector<int> pickup_ids = { pickup_id1, pickup_id2, pickup_id3};
    for(unsigned int i = 0; i < pickup_ids.size(); i++) {
      int pickup_id = pickup_ids[i];
    
    
    }


    if (pickup_id < 0 || pickup_id >= (int)centroids.size()) {
    ...
    goHome(ClientSocket);
    controlGripper(ClientSocket, GRIPPER_OPEN);
	  Pose pose_top = pose_from_centroid(centroids[pickup_id], -180, scaling, camera_orig_world);
    moveArm(ClientSocket, pose_top);
    Sleep(1000);
	  Pose pose_down = pose_from_centroid(centroids[pickup_id], -215, scaling, camera_orig_world);
    moveArm(ClientSocket, pose_down);
